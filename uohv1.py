...
    # Initialize hasher with optimal settings
...
    test_data = "Test data untuk hash final optimal"
...
        test_batch = [f"Data {i}" for i in range(size)]

        start = time.time()

        results = hasher.parallel_batch_hash(test_batch)

        total_time = time.time() - start

        print(f"Total time: {total_time:.4f} seconds")

        print(f"Average time per item: {total_time/size:.6f} seconds")

        print(f"Operations per second: {size/total_time:.2f}")
...
    print("Cache Hit Ratio: {:.2f}%".format(stats['cache_stats']['hit_ratio']))

    print("Memory Usage: {:.2f} MB".format(stats['memory_usage_mb']))

    

    print("\n=== Benchmark Results ===")

    bench_results = hasher.benchmark("benchmark_data")

    

    print("\nSingle Hash Performance:")

    for metric, value in bench_results['single_hash'].items():
...
    for size, metrics in bench_results['batch_processing'].items():
